...
import com.tz.jspstudy.test.Log4jTest;
...
public class ActionServlet extends HttpServlet {
	...
	private Log logger = LogFactory.getLog(ActionServlet.class);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		logger.info("已经进入ActionServlet sevice方法-准备分发请求");
	...
	public void init() throws ServletException {
		logger.info("初始化ActionServlet(所有的请求入口-准备加载struts-config配置文件)");
		//读取配置文件
		//获得项目的部署路径
		...
		logger.debug("初始化ActionServlet(所有的请求入口-加载struts-config配置文件完毕)");
	}
