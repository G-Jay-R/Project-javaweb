...
public class DispatchProcessor {
	...
	private Log logger = LogFactory.getLog(DispatchProcessor.class);
	...
	public void processor(HttpServletRequest request,
		...
		// 获得Action
		// 获得请求的路径地址
		logger.info("进入请求分发器DispatchProcessor--解析请求方法,去ActionConfig对象匹配");
		...
		logger.info("进入请求分发器DispatchProcessor--解析完请求方法,请求的路径为"+path);
		...
		try {
			logger.info("进入请求分发器--准备调用对应的业务处理Action");
